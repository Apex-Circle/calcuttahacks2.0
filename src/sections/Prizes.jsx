...
const PRIZES = () => (
  <section
    id="prizes"
    className="w-screen flex flex-col items-center py-[8vh] relative overflow-hidden bg-black text-white"
  >
    {/* Ambient blobs */}
    ...
    <div className="w-full max-w-6xl px-6 mt-[7vh] grid lg:grid-cols-3 gap-8 items-end z-10">
      {prizes.map((prize, idx) => (
        ...
        >
          ...
          >
            <div
              className={`absolute top-0 right-0 w-12 h-12 border-t-2 border-r-2 ${prize.color} opacity-50`}
            />
            <div className="flex flex-col items-center text-center">
              <div className="mb-6 transform group-hover:scale-110 transition-transform duration-300">
                {prize.icon}
              </div>
              <h3 className="rajdhani text-[1vw] text-gray-400 uppercase tracking-widest mb-1">
                {prize.place}
              </h3>
              <div className="orbitron text-[2vw] font-black mb-4 tracking-tight">
                {prize.amount}
              </div>
        ...
        <div className="relative border border-white/10 bg-neutral-950 p-8 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="rajdhani text-[0.85vw] text-gray-500">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full md:w-auto">
            {["Swag Kits", "Cloud Credits", "Certificates", "Food & Drinks"].map((item) => (
              <div
                key={item}
                className="flex items-center gap-2 rajdhani text-[0.85vw] text-gray-300 bg-white/5 px-4 py-2 border border-white/5"
              >
